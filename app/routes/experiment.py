...
async def get_experiment_id(experiment_id: int, db: Session = Depends(get_db)):
...
async def create_experiment(experiment: ExperimentPost, db: Session = Depends(get_db)):
...
async def add_researcher(experiment_id: int, researchers: ExperimentResearchers, db: Session = Depends(get_db)):
...
async def delete_researcher(experiment_id: int, researchers: ExperimentResearchers, db: Session = Depends(get_db)):
...
async def add_subjects(experiment_id: int, subjects: ExperimentSubjects, db: Session = Depends(get_db)):
...
async def get_all_experiments_researcher(researcher_id: int, db=Depends(get_db)):
